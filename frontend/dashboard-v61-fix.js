...
(function () {
  ...
  function paintUser(u, w) {
    u = u || {}; w = w || {};
    var rawName = u.first_name || u.firstname || u.given_name || u.prenom || u.full_name || u.name || '';
    var full = String(rawName || '').trim().split(/\s+/)[0] || '';
    if (full) full = full.charAt(0).toUpperCase() + full.slice(1).toLowerCase();
    var greet = $('dashWelcome'); if (greet) greet.textContent = full ? ('Bonjour ' + full + ' 👋') : 'Bonjour 👋';
    var plan = w.plan_label || w.plan || u.plan_label || u.plan || localStorage.getItem('pope_plan_label') || 'Free';
    var left = w.expert_left != null ? w.expert_left : (w.expert_remaining != null ? w.expert_remaining : (w.expert_requests_left != null ? w.expert_requests_left : (w.consultations_left != null ? w.consultations_left : (u.expert_left != null ? u.expert_left : (u.expert_remaining != null ? u.expert_remaining : (u.expert_requests_left != null ? u.expert_requests_left : '—'))))));
  ...
  function handleUrl() {
    ...
    // V61+ : au chargement initial après connexion, ne jamais restaurer automatiquement une étape avancée.
    // Les retours légitimes utilisent des paramètres URL explicites (from=app, step=3 depuis vault, etc.).
    return 1;
  }
  ...
  function init() {
    ...
    setTimeout(function () { patchPublicFns(); patchLinks(); showStep(target, { noScroll: true }); }, 1400);
  }
