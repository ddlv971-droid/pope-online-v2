...
(function(){
  ...
  function addReturnButtons(){
    ...
    if(nav && !byId('v53ReturnDashboard')){
      var a=document.createElement('a'); a.className='v40-topbar-btn'; a.id='v53ReturnDashboard'; a.href=dash+'?attach=last'; a.textContent='↩ Retour parcours';
    ...
    if(mob && !byId('v53ReturnDashboardMob')){
      var ma=document.createElement('a'); ma.id='v53ReturnDashboardMob'; ma.href=dash+'?attach=last'; ma.textContent='↩ Retour parcours'; ma.addEventListener('click',function(){saveCurrent();}); mob.insertBefore(ma,mob.firstChild);
    }
    var mission=byId('resultMissionLink'); if(mission) mission.style.display='none';
    var expert=byId('resultExpertLink'); if(expert){ expert.href=dash+'?attach=last'; expert.addEventListener('click',function(){saveCurrent();}); }
    var next=byId('nextActions');
    if(next && !byId('v53BackToJourney')){
      var b=document.createElement('a'); b.className='v36-next-btn'; b.id='v53BackToJourney'; b.href=dash+'?attach=last';
