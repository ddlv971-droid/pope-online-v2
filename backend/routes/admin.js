...
async function buildSatisfactionLink(client, user, adminId) {
  const frontendBase = resolveFrontendBase();
  const rawToken = randomToken(32);
  const tokenHash = sha256Hex(rawToken);
  await client.query(`update satisfaction_tokens set used_at=coalesce(used_at, now()) where user_id=$1 and used_at is null`, [user.id]);
  await client.query(`insert into satisfaction_tokens(user_id, token_hash, expires_at, created_by) values($1,$2,$3,$4)`, [user.id, tokenHash, nowPlusHours(24*30), adminId || null]);
  return `${frontendBase}/satisfaction.html?token=${encodeURIComponent(rawToken)}`;
}
