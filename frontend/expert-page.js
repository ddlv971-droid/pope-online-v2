...
function getUserKey(){
  ...
  // Fallback: lire depuis le cache localStorage pour affichage immédiat
  try{ var _cu=JSON.parse(localStorage.getItem('pope_session_user')||'null'); if(_cu?.id) return _cu.id; if(_cu?.email) return _cu.email; }catch{}
  return 'anonymous';
}
...
['subject','context','content'].forEach((id)=>{ const node=el(id); if(node){ node.addEventListener('input', persistDraft); node.addEventListener('change', persistDraft); }}); document.querySelectorAll('a[href*="vault.html"]').forEach((link)=>link.addEventListener('click', persistDraft)); el('archiveAttachmentSelect').addEventListener('change', ()=>{ updateAttachmentPreview(); persistDraft(); }); restoreDraft();
// Affichage immédiat de la card archive avec userId du cache localStorage
initArchiveChoices();
