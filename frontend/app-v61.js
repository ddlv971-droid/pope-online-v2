...
(function(){
  ...
  function targetUrl(){ return S.dashUrl(output()?3:2, !!output()); }
  function updateReturn(ready){ var href=targetUrl(); ['btnRetourDashboard','v53ReturnDashboard','v53ReturnDashboardMob','resultExpertLink','resultMissionLink','v53BackToJourney'].forEach(function(id){ var a=$(id); if(a){ a.href=href; a.onclick=function(){saveCurrent(); return true;}; }}); var info=$('v58RetourInfo'); if(info){ var st=S.load(); info.textContent = output() ? 'Draft prêt : retournez à l’étape Documents pour le joindre.' : (st.domain ? 'Domaine : '+st.domain+' — votre contexte est conservé.' : 'Retour à votre parcours conservé.'); } var b=$('v58DraftReady'); if(b && output()) b.classList.add('visible'); }
