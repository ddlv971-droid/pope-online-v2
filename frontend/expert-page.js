...
el('btnSend').addEventListener('click', async ()=>{ try { persistDraft(); const attachment=selectedArchive; const data=await apiFetch('/expert/request',{ method:'POST', body:{ subject:el('subject').value.trim(), context:el('context').value.trim(), content:el('content').value.trim(), vault_file_ids:selectedVaultIds(), generation_attachment: attachment ? { createdAt: attachment.createdAt || new Date().toISOString(), usecaseLabel: attachment.usecaseLabel || 'Génération IA', prompt: attachment.prompt || {}, result: attachment.result || '' } : null } }); setTicketsBadge(data.wallet); clearDraft(); el('msg').textContent = '✅ Votre demande a été transmise.'; showToast('Demande transmise', 'ok'); } catch(e){ console.error(e); if(e.status===402 && ['no_tickets','trial_expired','public_dossier_limit_reached','private_dossier_limit_reached'].includes(e.data?.error)){ el('msg').textContent='⚠️ Votre période gratuite est terminée ou votre quota gratuit est atteint. Contactez-nous pour définir l’offre adaptée à votre besoin.'; showToast('Accès temporairement limité', 'warn'); return; } el('msg').textContent='Erreur : '+getApiMessage(e); showToast('Envoi impossible', 'err'); } });
